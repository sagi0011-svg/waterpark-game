...
function rutiTick(hadBadEvent){
  ...
  else                         d -= 2;   // 55-62 — ראה "אזור מת" למטה
  if(hadBadEvent)              d += 10;
...
> **אזור מת 55–62 (התגלה בפלייטסט):** בטבלה המקורית הטווח הזה נותן דריפט של בדיוק 0, כלומר פארק שהפסיק להזיק — אבל לא הגיע ל-62 — משאיר את רותי נעוצה בשלב שאליו הגיעה לנצח. נוסף `else d -= 2`: עדיין צריך לעבור 62 כדי לדחוף אותה למטה באמת, אבל עמידה במקום כבר לא גזר דין. מדוד: משביעות רצון 58 היא יוצאת משלב 4 אחרי 7 חודשים; משביעות רצון 50 היא עדיין ננעצת ב-100. בפלייטסט זה הפך את "בלאגן · עונה שלמה" מפשיטת רגל בחודש 8 לסיום עונה מלא.
